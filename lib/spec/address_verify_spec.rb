...
require_relative "../address_verify/address_verify" # Assuming your script is named cli.rb
RSpec.describe "ADDRESSVERIFY" do
  let(:address_verify) { AddressVerify::AddressVerify.new }
  describe "#write_output_csv" do
    let(:valid_csv) { "lib/fixtures/valid_test_clients.csv" }
    let(:invalid_csv) { "lib/fixtures/invalid_test_clients.csv" }
    ...
    it "writes valid rows to the output CSV" do
      address_verify.write_output_csv(valid_csv)
    ...
    it "skips invalid rows and writes headers only once" do
      address_verify.write_output_csv(invalid_csv)
      expect(output_data.headers).to include("Email")
  ...
  describe "#run" do
    context "when --help option is provided" do
      it "prints help message and exits" do
        ...
        expect { address_verify.run }.to output(/Usage:/).to_stdout.and raise_error(SystemExit)
      end
    end
    ...
    context "when input CSV is not provided" do
      it "prints an error message and exits with status 1" do
        ...
        expect { address_verify.run }.to output(/Error: Please provide an input file/).to_stdout.and raise_error(SystemExit)
      end
    end
    ...
    context "when a valid input CSV is provided" do
      ...
      it "calls write_output_csv with the correct input" do
        ...
        expect { address_verify.run }.not_to raise_error
      end
    end
  end
  ...
  describe "#is_nil_data?" do
    it "returns true when all values are present" do
      ...
      expect(address_verify.is_nil_data?(data)).to be_falsy
    end

    it "returns false when some values are missing" do
      data = { "Name" => "John", "Email" => nil }
      expect(address_verify.is_nil_data?(data)).to be_truthy
    end
  end
  ...
  describe "#valid_email?" do
    it "returns true for a valid email" do
      ...
      expect(address_verify.valid_email?(email)).to be true
    end

    it "returns false for an invalid email" do
      email = "test@example"
      expect(address_verify.valid_email?(email)).to be false
    end
  end
  ...
  describe "#geocode_location" do
    it "returns true for a valid address" do
      ...
      expect(address_verify.geocode_location(data, "Postal")).to be_falsy
    end
    ...
    it "returns false for an invalid address" do
      ...
      expect(address_verify.geocode_location(data, "Residential")).to be_truthy
    end
  end
  describe "#print_help" do
    it "prints the help message to STDOUT" do
      allow(address_verify).to receive(:puts)

      address_verify.print_help
      ...
      expect(address_verify).to have_received(:puts).with(expected_help_message)
    end
  end
end
